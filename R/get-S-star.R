...
get_S_star <- function(
  ...
  no_brood_populations <- dimnames(E_star)$i[
    !dimnames(E_star)$i %in% c(brood_population, aggregate_population)
  ]
  ...
  for (y in 1:n_years) {
    for (a in 1:n_ages) {
      S_star[aggregate_population, y, a] = E_star[aggregate_population, y, a] -
        B_star[y, a] # aggregate
      S_star[brood_population, y, a] = E_star[brood_population, y, a] -
        B_star[y, a] # Kitsumkalum
  ...
  d$a <- as.integer(d$a)
  if (save_csv == TRUE) {
    write.csv(d, paste0(save_path, "/S-star.csv"), row.names = FALSE)
  }
