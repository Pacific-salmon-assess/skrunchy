...
get_stat_week <- function(date) {
  ...
  stat_week <- ifelse(
    strftime(
      ISOdate(as.integer(strftime(date, format = "%Y")), 1, 1),
      format = "%A"
    ) ==
      "Sunday", # is the first day of the given year a Sunday?
    ...
    as.integer(strftime(date, format = "%U")) + 1
  ) # If January 1 is not a Sunday (in the given year), then %U format week starts at 0 and you have to add 1
}
