...
get_E_star <- function(
  ...
  if (
    !isTRUE(
      identical(dim(E)[2], dim(omega)[2]) &&
        identical(dim(E)[2], dim(K_star)[1])
    )
  ) {
  ...
  if (
    !all(
      dimnames(E)$y %in% dimnames(omega)$y,
      dimnames(E)$y %in% dimnames(K_star)$y
    )
  ) {
  ...
  populations_use_omega <- dimnames(E)$i[
    !dimnames(E)$i == population_use_age_from_river_samples
  ]
  n_ages <- dim(omega)[3]
  for (y in 1:n_years) {
    for (a in 1:n_ages) {
  ...
  if (add_6_7 == TRUE) {
    # add age 7 to age 6 fish by return year
    if (any(dimnames(E_star)$a == "7")) {
      ...
      E_star_add_6_7 <- E_star_add_6_7[,,
        -grep("7", dimnames(E_star_add_6_7)$a)
      ] # remove age 7 escapement dimension
  ...
  d$b <- d$y - d$a
  if (save_csv == TRUE) {
    write.csv(d, paste0(save_path, "/E-star.csv"), row.names = FALSE)
  }
