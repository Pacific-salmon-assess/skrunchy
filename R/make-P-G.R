...
make_P_G <- function(
  ...
  if (!is.na(n_years)) {
    years <- seq(start_year, length.out = n_years)
  } # years
  ...
  P <- sapply(
    ...
    FUN = function(x) {
      fdat <- replicate(
        n_weeks,
        sample(3:5, size = n_populations, replace = TRUE)
      )
      fdat1 <- apply(fdat, 2, FUN = function(x) {
        {
          x / sum(x)
        } *
          100
  ...
  sigma_P <- array(
    data = runif(n = n_populations * n_weeks * n_years, min = 1, max = 5),
  ...
  G <- array(
    sample(catch_values, n_weeks * n_years, replace = TRUE),
    dim = c(n_weeks, n_years),
