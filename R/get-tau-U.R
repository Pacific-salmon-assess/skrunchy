...
#'
#' @param Tau_U Numeric, vector of total terminal mortalities in the upper Skeena (upstream of Terrace), by year. Combination of recrational and First Nations FSC. Note uppercase Tau denotes total not broken out by population or age.
...
get_tau_U <- function(
  ...
  if (
    !isTRUE(
      identical(length(Tau_U), dim(omega)[2]) &&
        identical(length(Tau_U), dim(P_tilde)[2])
    )
  ) {
  ...
  if (
    !all(
      dimnames(omega)$i[!dimnames(omega)$i == "Skeena"] %in% dimnames(P_tilde)$i
    )
  ) {
  ...
  for (a in 1:n_ages) {
    for (y in 1:n_years) {
      tau_U[aggregate_population, y, a] <- Tau_U[y] *
        omega[aggregate_population, y, a]
  ...
  if (add_6_7 == TRUE) {
    # add age 7 to age 6 fish by brood year
    if (any(dimnames(tau_U)$a == "7")) {
      # only do this if there are actual age 7 fish in the data
      ...
      for (y in 1:(n_years - 1)) {
        tau_U_add_6_7[, y, "6"] <- tau_U_add_6_7[, y, "6"] +
          tau_U_add_6_7[, y + 1, "7"] # add age 7 mortalities to age 6 by brood year (age 7 mortalities from age 6 return year +1)
      }
