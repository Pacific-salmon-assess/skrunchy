...
get_K_star <- function(
  ...
  if (
    !isTRUE(
      identical(dim(omega_KM)[2], dim(omega_KF)[2]) &&
        identical(dim(omega_KM)[2], dim(H_star1)[2])
    )
  ) {
  ...
  if (
    !all(
      dimnames(omega_KM)$a %in% dimnames(omega_KF)$a,
      dimnames(omega_KM)$a %in% dimnames(H_star1)[[2]]
    )
  ) {
  ...
  if (
    !isTRUE(
      identical(length(K), length(y_K)) &&
        identical(length(K), length(K_M)) &&
        identical(length(K), length(K_F)) &&
        identical(length(K), length(H)) &&
        identical(length(K), dim(omega_KM)[1]) &&
        identical(length(K), dim(omega_KF)[1]) &&
  ...
  if (
    !all(
      dimnames(omega_KM)$y %in% dimnames(omega_KF)$y,
      dimnames(omega_KM)$y %in% dimnames(H_star1)[[1]]
    )
  ) {
  ...
  d$a <- as.integer(d$a)
  if (save_csv == TRUE) {
    write.csv(d, paste0(save_path, "/K-star.csv"), row.names = FALSE)
  }
