...
get_p <- function(
  ...
  n_ages <- dim(W_star)[3]
  no_hatchery_populations <- dimnames(W_star)$i[
    !dimnames(W_star)$i %in% c(hatchery_population, aggregate_population)
  ]
  ...
  for (y in 1:n_years) {
    for (a in 1:n_ages) {
      ...
      p[hatchery_population, y, a] <- (W_star[hatchery_population, y, a] +
        B_star[y, a]) /
        E_star[hatchery_population, y, a]
      p[aggregate_population, y, a] <- (W_star[aggregate_population, y, a] +
        B_star[y, a]) /
        E_star[aggregate_population, y, a]
