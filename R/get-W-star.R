...
get_W_star <- function(
  ...
  no_hatchery_populations <- dimnames(S_star)$i[
    !dimnames(S_star)$i %in% c(hatchery_population, aggregate_population)
  ]
  ...
  for (y in 1:n_years) {
    for (a in 1:n_ages) {
      W_star[aggregate_population, y, a] <- S_star[aggregate_population, y, a] -
        H_star[y, a] # aggregate
      W_star[hatchery_population, y, a] <- S_star[hatchery_population, y, a] -
        H_star[y, a] # Kitsumkalum
      W_star[no_hatchery_populations, y, a] <- S_star[
        no_hatchery_populations,
        y,
        a
      ] # all others
    }
