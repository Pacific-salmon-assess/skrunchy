...
get_tau_L <- function(
  ...
  if (
    !isTRUE(
      identical(length(Tau_L), dim(omega)[2]) &&
        identical(length(Tau_L), dim(P_tilde)[2])
    )
  ) {
  ...
  not_aggregate_populations <- dimnames(omega)$i[
    !dimnames(omega)$i == aggregate_population
  ]
  ...
  for (a in 1:n_ages) {
    for (y in 1:n_years) {
      tau_L[aggregate_population, y, a] <- Tau_L[y] *
        omega[aggregate_population, y, a]
      tau_L[not_aggregate_populations, y, a] <- Tau_L[y] *
        omega[not_aggregate_populations, y, a] *
        P_tilde[not_aggregate_populations, y] /
        sum_P_tilde[y]
    }
  }
  if (add_6_7 == TRUE) {
    # add age 7 to age 6 fish by brood year
    if (any(dimnames(tau_L)$a == "7")) {
      ...
      for (y in 1:(n_years - 1)) {
        tau_L_add_6_7[, y, "6"] <- tau_L_add_6_7[, y, "6"] +
          tau_L_add_6_7[, y + 1, "7"] # add age 7 mortalities to age 6 by brood year (age 7 mortalities from age 6 return year +1)
      }
