...
get_P_tilde <- function(
  ...
  if (
    !all(
      dimnames(P)$y %in% dimnames(sigma_P)$y,
      dimnames(P)$y %in% dimnames(G)$y
    )
  ) {
  ...
  P_tilde <- abind::abind(P_tilde_list, along = 2) # bind list into array
  ...
  if (save_csv == TRUE) {
    write.csv(d, paste0(save_path, "/P-tilde.csv"), row.names = FALSE)
  }
