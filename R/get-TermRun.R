...
get_TermRun <- function(
  ...
  if (
    !isTRUE(
      identical(dim(tau_W)[2], dim(W_star)[2]) &&
        identical(dim(tau_W)[2], dim(B_star)[1])
    )
  ) {
  ...
  if (
    !all(
      dimnames(tau_W)$y %in% dimnames(W_star)$y,
      dimnames(tau_W)$y %in% dimnames(B_star)$y
    )
  ) {
  ...
  if (
    !isTRUE(
      identical(dim(tau_W)[3], dim(W_star)[3]) &&
        identical(dim(tau_W)[3], dim(B_star)[2])
    )
  ) {
  ...
  if (
    !all(
      dimnames(tau_W)$a %in% dimnames(W_star)$a,
      dimnames(tau_W)$a %in% dimnames(B_star)$a
    )
  ) {
    stop("Age (a) values are not equal.")
  }

  no_brood_populations <- dimnames(W_star)$i[
    !dimnames(W_star)$i %in% c(brood_population, aggregate_population)
  ]
  ...
  for (y in years) {
    for (a in ages) {
      TermRun[brood_population, y, a] <- tau_W[brood_population, y, a] +
        W_star[brood_population, y, a] +
        B_star[y, a]
      TermRun[aggregate_population, y, a] <- tau_W[aggregate_population, y, a] +
        W_star[aggregate_population, y, a] +
        B_star[y, a]
      TermRun[no_brood_populations, y, a] <- tau_W[no_brood_populations, y, a] +
        W_star[no_brood_populations, y, a]
    }
  }
  d <- as.data.frame.table(
    TermRun,
    responseName = "TermRun",
