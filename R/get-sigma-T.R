#' Get standard error for T, return to Terrace
#'
#' @param T Array of returns to Terrace with two dimensions: population (i) and year (y).
...
#'
#' @export
get_sigma_T <- function( T, K, sigma_K, P_tilde, sigma_P_tilde, aggregate_population = TRUE) {
   if(aggregate_population == TRUE) {
        variance <- T^2 * ( sigma_K / K^2 + sigma_P_tilde / P_tilde^2 )
        SE <- sqrt(variance)
        SE
   } else {
