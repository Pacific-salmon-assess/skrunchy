...
#'
#' @examples
#' scale_age <- c("31", "31", "32", "42", "52",
#'                "62", "1M", "2M", "3M", "4M", NA, NA)
...
#' d$revised_stage <- get_stage( scale_age = scale_age,
#'                                cwt_age = cwt_age,
#'                                POH = POH,
#'                                comments = comments)
#'
...
get_stage <- function(
  ...
  # check lengths equal
  if (
    !all(length(scale_age) == c(length(cwt_age), length(POH), length(comments)))
  ) {
  ...
  for (i in 1:length(scale_age)) {
    if (!is.na(cwt_age[i])) {
      # If there is a cwt age, ignore everything else
      stage[i] <- ifelse(isTRUE(cwt_age[i] < 4), "jack", "adult")
    } # If CWT age is less than 4, it is a jack
    if (is.na(cwt_age[i])) {
      # If cwt age is NA, use the other data sources
      stage[i] <- ifelse(
        isTRUE(cwt_age[i] < 4),
        "jack", # If CWT age is less than 4, it is a jack
        ifelse(
          scale_age[i] %in% jack_ages,
          "jack", # If scale age is a jack age, it is a jack
          ifelse(
            scale_age[i] %in% adult_ages,
            "adult", # if scale age is an adult age, it is an adult.
            ...
            ifelse(
              all(!is.na(POH[i]), POH[i] <= jack_cutoff_length_POH),
              "jack",
              # If there is no length, scale age, or CWT age, and it is jack in comments, it is a jack
              ifelse(
                all(
                  is.na(POH[i]) && is.na(scale_age[i]),
                  is.na(cwt_age[i]) && grepl("Jack|jack", comments[i])
                ),
                "jack",
                "adult"
              )
