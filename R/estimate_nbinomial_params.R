#' Estimate parameters for binomial sampler
#'
...
#'
#' @returns List with three elements: (1) Numeric, mu parameter. (2) Numeric, size parameter. (3) Numeric, prob, mu / (size / mu).
#'
#' @export
#'
#' @examples
#' params <- estimate_nbinomial_params(ex_k$kitsumkalum_escapement)

estimate_nbinomial_params <- function(X) {
  if (mean(X) >= var(X)) {
    stop(
      "Mean of the dataset is greater than or equal to its variance; consider a Poisson distribution"
    )
  }
