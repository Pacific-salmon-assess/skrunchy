...
ex_k <- read.csv(here::here("data-raw/kitsumkalum-escapement.csv"))

# get return to Terrace
X <- get_X(
  P_tilde = ex_P_tilde,
  sigma_P_tilde = ex_sigma_P_tilde,
...
ad <- sapply(p_ages, FUN = function(x) {
...
ad_jacks <- sapply(p_ages_jacks, FUN = function(x) {
  rpois(n = n_populations * n_years, lambda = x)
})
ex_n_with_jacks <- array(
  ad_jacks,
  dim = c(n_populations, n_years, n_ages_jacks),
  dimnames = list(i = populations, y = years, a = ages_jacks)
)

# Make up age sampling data for Kitsumkalum males and females
ad_males <- sapply(p_ages_males, FUN = function(x) {
  rpois(n = n_years, lambda = x)
})
ex_n_males <- array(
  ad_males,
  dim = c(n_years, n_ages),
  dimnames = list(y = years, a = ages)
)
ad_females <- sapply(p_ages_females, FUN = function(x) {
  rpois(n = n_years, lambda = x)
})
ex_n_females <- array(
  ad_females,
...
ex_Tau_U_total <- get_Tau_U_total(
  omega_J = ex_omega_J,
...
ksd <- read.csv(here::here("data-raw", "K_star.csv"))
...
dim_position_K_star <- sapply(dim_order_K_star, function(x) {
  grep(paste0("^", x, "$"), names(ksdl))
})
K_star <- tapply(
  ksdl$K_star,
  ksdl[, dim_position_K_star],
  FUN = print,
  default = 0
)
ex_K_star <- K_star
...
brood_ages <- c(4, 5, 6)
B_star <- array(
  sample(1:20, size = n_years * n_ages, replace = TRUE),
  dim = c(n_years, length(brood_ages)),
  dimnames = list(y = years, a = brood_ages)
)
...
H_star <- S_star$S_star["Kitsumkalum", , ] *
  runif(n = length(S_star$S_star["Kitsumkalum", , ]), 0.01, 0.2)
ex_H_star <- H_star
...
ex_Tau_L_total <- get_Tau_L_total(
  omega_J = ex_omega_J,
  tyee = ex_Tau$tyee,
