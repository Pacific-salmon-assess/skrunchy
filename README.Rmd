...
The `data/` folder contains example data that all made up (exception is Kitsumkalum River escapement data), so don't use this for any analysis. This is only for package testing and examples. Example data goes from return years 1984-2020.
...
Note that most of the package functions produce lists with two elements. where the first element is an array, and the second element is a data frame. The array is used for subsequent analysis, and the data frame is useful for plotting and producing report tables. 

Read in example data for Skeena Tyee test fishery weekly catch and genetic mixture data. 
