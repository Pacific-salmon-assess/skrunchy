test_that("Total mortalities by year populations sum to aggregate population", {
  ...
  expect_equal(
    apply(tau_U$tau_U["Skeena", , ], 1, sum),
    apply(tau_U$tau_U[setdiff(populations, "Skeena"), , ], 2, sum)
  )
  expect_equal(as.numeric(apply(tau_U$tau_U["Skeena", , ], 1, sum)), Tau_U)
  # test adding age 7 to age 6 by brood year
  tau_U_add_6_7 <- get_tau_U(
    Tau_U = Tau_U,
    omega = omega$omega,
    P_tilde = P_tilde$P_tilde,
    aggregate_population = "Skeena",
  ...
  expect_equal(
    tau_U_add_6_7$tau_U[, 1:(n_years - 1), "6"],
    tau_U$tau_U[, 1:(n_years - 1), "6"] + tau_U$tau_U[, 2:(n_years), "7"]
  )
})
